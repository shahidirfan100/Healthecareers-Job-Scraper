...
This Apify actor scrapes healthcare job listings from Healthecareers.com using Crawlee's CheerioCrawler and gotScraping.

## Features

- Scrapes Healthecareers search results and job detail pages (no browser required).
...
- `location` (string) — Location filter (Healthecareers listings are typically US-based but this is kept for consistency).
- `category` (string) — Job category to filter (if available on Healthecareers).
- `startUrl` / `url` / `startUrls` — Specific Healthecareers search URL(s) to start from. If provided, these override keyword/location.
...
- If Healthecareers changes their markup, selectors in `src/main.js` may need small updates.